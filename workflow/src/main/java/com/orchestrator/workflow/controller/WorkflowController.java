...
getExecutableTasks(
    ...
    @GetMapping("/{id}")
    public ResponseEntity<WorkflowDetailsDTO> getWorkflowById(@PathVariable Long id)
    {
        WorkflowDetailsDTO workflow=ws.getWorkflowById(id);
    ...
    {
        ws.deleteWorkflow(id);
        return ResponseEntity.ok("Workflow deleted.");
    }
    
    @PostMapping("/{id}/clone")
public ResponseEntity<String> cloneWorkflow(
        @PathVariable Long id
){

    ws.cloneWorkflow(id);

    return ResponseEntity.ok("Workflow cloned.");

}

@PutMapping("/{id}")
public ResponseEntity<String> updateWorkflow(@PathVariable Long id, @RequestBody WorkflowRequestDTO request)
{
    ws.updateWorkflow(id,request);
